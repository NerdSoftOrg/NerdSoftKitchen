...
public class GrillTableBlockEntityRenderer implements BlockEntityRenderer<GrillTableBlockEntity> {
    ...
    public void render(@NotNull GrillTableBlockEntity blockEntity, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        boolean pastLod1 = ScreenSpaceLod.isPastThreshold(packedPos, LodConfig.ITEM_RADIUS_SQR, distanceSqr, LodConfig.ITEM_LOD_COVERAGE_THRESHOLD);
    ...
    private void renderGrillSlot(GrillTableBlockEntity blockEntity, int slot, ItemStack stack, int facing2D,
        ...
        ItemLodRenderer.renderItem(this.itemRenderer, stack, ItemDisplayContext.FIXED, poseStack, bufferSource, packedLight, packedOverlay, renderLevel, seedBase + slot, pastLod1);
        poseStack.popPose();
    }

    private void renderCampfireSlot(int slot, ItemStack stack, int facing2D, int seedBase,
